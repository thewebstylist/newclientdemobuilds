#!/usr/bin/env python3
"""Build the single-file versions of the site.

    python3 tools/build_embed.py

Writes two files, each containing all the markup, CSS and JavaScript:
  * wordpress/pinkladycreamery-homepage-embed.html: a paste-ready block for a
    WordPress / Elementor HTML widget (details below).
  * wordpress/pinkladycreamery-standalone.html: the complete page as one HTML
    document, with the <head> SEO tags, that opens and works on its own.
Every photo already loads from the Pink Lady WordPress media library, so
nothing needs to be inlined.
...
STANDALONE = os.path.join(ROOT, "wordpress", "pinkladycreamery-standalone.html")
SCOPE = ".plc"
...
def build_standalone():
    ...
    js_tag = '<script src="assets/js/main.js" defer></script>'
    assert css_link in html and js_tag in html, "index.html asset tags changed; update build_embed.py"
    html = html.replace(css_link, "<style>\n" + read("assets/css/style.css") + "</style>")
    # Inline scripts can't defer, so the script sits at the end of <body>, after the markup it needs.
    html = html.replace(js_tag, "<script>\n" + read("assets/js/main.js") + "</script>")
    assert "assets/" not in html, "unreplaced local path"
    return html
...
def write(path, text):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(text)
    print(f"wrote {os.path.relpath(path, ROOT)} ({len(text.encode()) / 1024:.0f} KB)")


def main():
    write(STANDALONE, build_standalone())
